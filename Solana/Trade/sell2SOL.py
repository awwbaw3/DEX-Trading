...
async def sell_normal(solana_client, TARGET_TOKEN, payer):
    retry_count = 0
    while retry_count < MAX_RETRIES:
        try:
            # token_symbol, SOl_Symbol = getSymbol(TARGET_TOKEN)
            ...
            swap_tx.add(instructions_swap,set_compute_unit_price(COMPUTEUNITPRICE),set_compute_unit_limit(COMPUTEUNITLIMIT))
...
async def main():
    ...
    print(payer.pubkey())
    await sell_normal(solana_client, target_Token, payer)
